...
/*const shipDatas = [ //сами кораблики, их размеры, и первоночальное расположение
   { size: 4, direction: "row", startX: 10, startY: 345 },
   { size: 3, direction: "row", startX: 10, startY: 390 },
   { size: 3, direction: "row", startX: 120, startY: 390 },
   { size: 2, direction: "row", startX: 10, startY: 435 },
...
class PreparationScene extends Scene { //подготовочная сцена, этап
   draggedShip = null; // свойство на перетаскиваемый корабль
   draggedOffsetX = 0; //смещение относительно левого угла
   draggedOffsetY = 0;

   removeEventListeners = [];

   init() {
      // const { player } = this.app; //достаем player из приложения
      // for (const { size, direction, startX, startY } of shipDatas) {
      //    const ship = new ShipView(size, direction, startX, startY);
      //    player.addShip(ship); //добаляем корабль
      // }
      this.manually();
   }
   start() {
   ...
   update() {
      const { mouse, player } = this.app;
      if (!this.draggedShip && mouse.left && !mouse.pLeft) //если нет перетаскиваемого корабля, но зажата мышь
      {
         const ship = player.ships.find((ship) => ship.isUnder(mouse));

         if (ship) {
            const shipRect = ship.div.getBoundingClientRect();
            this.draggedShip = ship; //это тот корабль который перетаскивают
            this.draggedOffsetX = mouse.x - shipRect.left;
            this.draggedOffsetY = mouse.y - shipRect.top;
      ...
      if (mouse.left && this.draggedShip) //перетаскивание
      {
         const { left, top } = player.root.getBoundingClientRect();
         const x = mouse.x - left - this.draggedOffsetX;
         const y = mouse.y - top - this.draggedOffsetY;
