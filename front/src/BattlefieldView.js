...
/*
class BattlefieldView extends Battlefield {
	root = null; //все приложение
	table = null; //поле
	dock = null; //все корабли которые есть
	polygon = null; //для выстрелов все выстрелы

	cells = []; //для быстрого доступа к каждой ячейки
	...
	constructor() {
		...
		root.append(table, dock, polygon); //устанавливаем дочерние элементы объекта

		for (let y = 0; y < 10; y++) //матрица наполнения таблицы, срздание полей
		{
			const row = [];
			const tr = document.createElement('tr');
			tr.classList.add("battlefield-row");
			...
			for (let x = 0; x < 10; x++) {
				...
				tr.append(td); //добавли на страницу
				row.push(td); //добавили в строку
			}
			table.append(tr);
			this.cells.push(row);
		...
		for (let x = 0; x < 10; x++) //создание маркеров на карте
		{
			const cell = this.cells[0][x];
			const marker = document.createElement("div");
			marker.classList.add("marker", "marker-column");
		...
		for (let y = 0; y < 10; y++) {
			const cell = this.cells[y][0]; //пробигаемся сначало по первому чтолбцу
			const marker = document.createElement("div");
			marker.classList.add("marker", "marker-row");
			marker.textContent = y + 1; //цифры в столбик
			cell.append(marker);
		}
	}

	addShip(ship, x, y) //переопределяем частично addShip, для визуалки
	{
		if (!super.addShip(ship, x, y)) //если false то не добавляем корабль
		{
			return false;
		}
		this.dock.append(ship.div); //иначе добавляем в dock

		if (ship.placed) //если корабль на игровом поле
		{
			const cell = this.cells[y][x];
			const cellRect = cell.getBoundingClientRect();
			const rootRect = this.root.getBoundingClientRect();
	...
	isUnder(point) //уже как результат находится ли курсор над элементом
	{
		return isUnderPoint(point, this.root);
	}
}
